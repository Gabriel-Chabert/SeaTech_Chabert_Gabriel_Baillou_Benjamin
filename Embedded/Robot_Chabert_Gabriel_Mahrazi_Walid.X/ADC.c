...
void InitADC1(void) {
    ...
    //On prend 12 bits pour plus de résolution
    AD1CON1bits.AD12B = 1; // 0 : 10bits - 1 : 12bits
    //Comme le microcontroleur est un 16 bits, on doit lui dire d'utiliser seulement 12 bits sur les 
    AD1CON1bits.FORM = 0b00; // 00 = Integer (DOUT = 0000 dddd dddd dddd)
    //pas le même nombre de bit entre CAN et microcontroleur
    ...
    //On définit le nombre d'entrée analogique qu'on va utiliser (2+1)
    ...
    //AD1CON3
    /************************************************************/
    //Timing pour laisser le temps de faire les convertion(système contenant un condensateur qui prend la valeur de la convertion)
    ...
    /************************************************************/
    //ADC utilisé : 8(B8)-9(B9)-10(B10)-0(B0)-11(B11)
    ANSELBbits.ANSB0 = 1; //carte entrée "ANA_0-B0"
    ANSELBbits.ANSB8 = 1; //carte entrée "ANA_8-B8"
    ANSELBbits.ANSB9 = 1; //carte entrée "ANA_9-B9"
    ANSELBbits.ANSB10 = 1; //carte entrée "ANA_10-B10"
    ANSELBbits.ANSB11 = 1; //carte entrée "ANA_11-B11"
...
unsigned char ADCIsConversionFinished(void)//On regarde si la convertion est bien terminée. Alors on peut en relancer une autre.
{
